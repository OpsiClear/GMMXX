"""Inference under backend='cuda' for diagonal covariance.
...
@pytest.fixture
def fitted_diag_cuda_gmm():
    from gmmxx import GMMXX
    torch.manual_seed(0)
    x = torch.randn(2048, 16, device="cuda")
    gmm = GMMXX(n_components=8, max_iter=15, tol=1e-4, random_state=0,
                covariance_type="diag", backend="cuda")
    gmm.fit(x)
    return gmm, x


def test_predict_returns_correct_shape_dtype(fitted_diag_cuda_gmm):
    gmm, x = fitted_diag_cuda_gmm
...
def test_predict_proba_sums_to_one(fitted_diag_cuda_gmm):
    gmm, x = fitted_diag_cuda_gmm
...
def test_score_samples_returns_finite(fitted_diag_cuda_gmm):
    gmm, x = fitted_diag_cuda_gmm
...
def test_score_returns_finite_scalar(fitted_diag_cuda_gmm):
    gmm, x = fitted_diag_cuda_gmm
    s = gmm.score(x[:512])
...
def test_predict_consistent_with_predict_proba(fitted_diag_cuda_gmm):
...
def test_per_feature_variance_shape(fitted_diag_cuda_gmm):
    """Diag covariance: covariances_ should be (K, D) per-feature."""
    gmm, _ = fitted_diag_cuda_gmm
    assert gmm.covariances_.shape == (gmm.k, 16)
    assert (gmm.covariances_ > 0).all()
