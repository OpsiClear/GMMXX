...
//
// log p_k(x) = log_w_k - 0.5 * Σ_d log(2π σ_{k,d}²) - 0.5 * Σ_d (x_d − μ_{k,d})²/σ_{k,d}²
template <typename T>
__global__ void __launch_bounds__(128)
diag_assign_safe_kernel(
    ...
    const float* var_b = var + (size_t)b * K * D;
    ...
    for (int k = 0; k < K; ++k) {
        ...
        const float* var_kd = var_b + (size_t)k * D;
        float log_det = 0.0f;
        float dist = 0.0f;
        for (int d = 0; d < D; ++d) {
            float v = var_kd[d];
...
diag_logsumexp_safe_kernel(
    ...
    if (k < K) {
        ...
        logit = log_w_b[k] - 0.5f * log_det - 0.5f * dist;
    }
...
void _check_inputs(const at::Tensor& x, const at::Tensor& means,
    ...
    TORCH_CHECK(var.dim() == 3, "var must be (B, K, D)");
    TORCH_CHECK(var.size(0) == x.size(0), "var batch dim mismatch");
    TORCH_CHECK(var.size(1) == means.size(1), "var K dim mismatch");
    TORCH_CHECK(var.size(2) == means.size(2), "var D dim mismatch");
    TORCH_CHECK(log_w.dim() == 2, "log_w must be (B, K)");
    TORCH_CHECK(log_w.sizes() == at::IntArrayRef({var.size(0), var.size(1)}),
                "log_w must be (B, K)");
...
void launch_assign(const at::Tensor& x, const at::Tensor& means,
    ...
    diag_assign_safe_kernel<T><<<grid, kThreads, 0, stream>>>(
...
void launch_logsumexp(const at::Tensor& x, const at::Tensor& means,
    ...
    dim3 grid(N, B);
    diag_logsumexp_safe_kernel<T><<<grid, threads, smem_bytes, stream>>>(
        x.data_ptr<T>(), means.data_ptr<T>(),
        var.data_ptr<float>(), log_w.data_ptr<float>(),
        out.data_ptr<float>(),
        B, N, K, D
...
void launch_resp(const at::Tensor& x, const at::Tensor& means,
    ...
    diag_resp_safe_kernel<T><<<grid, kThreads, 0, stream>>>(
        x.data_ptr<T>(), means.data_ptr<T>(),
        var.data_ptr<float>(), log_w.data_ptr<float>(),
...
at::Tensor assign_safe(const at::Tensor& x, const at::Tensor& means,
    ...
    switch (x.scalar_type()) {
        ...
        default: TORCH_CHECK(false, "diag.assign_safe: unsupported dtype ", x.scalar_type());
    }
    return result;
}
...
at::Tensor logsumexp_safe(const at::Tensor& x, const at::Tensor& means,
    ...
    switch (x.scalar_type()) {
        ...
        default: TORCH_CHECK(false, "diag.logsumexp_safe: unsupported dtype ", x.scalar_type());
    }
    return result;
}
...
at::Tensor resp_safe(const at::Tensor& x, const at::Tensor& means,
    ...
    switch (x.scalar_type()) {
        ...
        default: TORCH_CHECK(false, "diag.resp_safe: unsupported dtype ", x.scalar_type());
    }
    return result;
}
