...
void blocked_update(const at::Tensor& x,
    ...
    TORCH_CHECK(sumsq_out.sizes() == at::IntArrayRef({B, K, D}),
                "sumsq_out must be (B,K,D)");  // KEY DIFFERENCE: was (B,K) for spherical
    ...
    switch (x.scalar_type()) {
        case at::kFloat:
            blocked_update_diag_kernel<float><<<grid, kThreads, 0, stream>>>(
            ...
            break;
        case at::kHalf:
            blocked_update_diag_kernel<at::Half><<<grid, kThreads, 0, stream>>>(
                x.data_ptr<at::Half>(), cluster_ids.data_ptr<int32_t>(),
                sums_out.data_ptr<float>(), sumsq_out.data_ptr<float>(),
                counts_out.data_ptr<int32_t>(), B, N, K, D);
            break;
        case at::kBFloat16:
            blocked_update_diag_kernel<at::BFloat16><<<grid, kThreads, 0, stream>>>(
                x.data_ptr<at::BFloat16>(), cluster_ids.data_ptr<int32_t>(),
                sums_out.data_ptr<float>(), sumsq_out.data_ptr<float>(),
                counts_out.data_ptr<int32_t>(), B, N, K, D);
            break;
        default:
            TORCH_CHECK(false, "blocked_update_diag: unsupported dtype ", x.scalar_type());
    }
