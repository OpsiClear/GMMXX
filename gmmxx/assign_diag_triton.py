...
def _diag_config(n: int, k: int, d: int) -> dict[str, int]:
    if d <= 32:
        return {"BLOCK_N": 128, "BLOCK_K": 64, "num_warps": 4, "num_stages": 1}
    if d <= 64 and k >= 128:
        return {"BLOCK_N": 16, "BLOCK_K": 64, "num_warps": 4, "num_stages": 1}
    return {"BLOCK_N": 32, "BLOCK_K": 64, "num_warps": 4, "num_stages": 1}


def diag_logsumexp_triton(
    x: torch.Tensor,
    ...
    *,
    ...
    selected = _diag_config(N, K, D) if config is None else config
    grid = (triton.cdiv(N, selected["BLOCK_N"]), B)
    _diag_logsumexp_kernel[grid](
        x,
        ...
        precision.stride(1),
        precision.stride(2),
        weighted_means.stride(0),
        weighted_means.stride(1),
        weighted_means.stride(2),
...
def diag_assign_triton(
    ...
    *,
    ...
    assert log_weights.shape == (B, K), "log_weights shape mismatch"
    if D < 16:
        raise ValueError(f"diag_assign_triton requires D >= 16, got D={D}")
    if out is None:
        out = torch.empty((B, N), device=x.device, dtype=torch.int32)

    selected = _diag_config(N, K, D) if config is None else config
    grid = (triton.cdiv(N, selected["BLOCK_N"]), B)
    _diag_assign_kernel[grid](
        x,
        precision.to(torch.float32),
        weighted_means.to(torch.float32),
        mean_precision_mean.to(torch.float32),
...
def diag_resp_triton(
    ...
    *,
    ...
    if D < 16:
        raise ValueError(f"diag_resp_triton requires D >= 16, got D={D}")
    if out is None:
